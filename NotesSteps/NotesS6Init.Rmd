...
df2$indep <- df2$indep1
fm1 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep2
fm2 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep3
fm3 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep4
fm4 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep5
fm5 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep6
fm6 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep7
fm7 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
df2$indep <- df2$indep8
fm8 <- felm(epats ~ indep| Time * cusip | 0 | 0, data = df2)
...
stargazer(m1, title = "Results of regression with dummies, orig def. p1", type = 'latex', digits = NA, add.lines=list(c("Time FE", "X", "X", "X"), c("cusip FE", "X", "X", "X")))
stargazer(m2, title = "Results of regression with dummies, orig def. p2", type = 'latex', digits = NA, add.lines=list(c("Time FE", "X", "X", "X"), c("cusip FE", "X", "X", "X")))
stargazer(m3, title = "Results of regression with dummies, orig def. p3", type = 'latex', digits = NA, add.lines=list(c("Time FE", "X", "X"), c("cusip FE", "X", "X")))
```